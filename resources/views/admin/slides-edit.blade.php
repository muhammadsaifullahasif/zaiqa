...
<div class="main-content-inner">
    ...
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" id="form-edit-slide" action="{{ route('admin.slides.update', $slide->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <fieldset class="name">
                    <div class="body-title">Tagline <span class="tf-color-1">*</span></div>
                    <input class="flex-grow" type="text" placeholder="Tagline" name="tagline" tabindex="0" value="{{ $slide->tagline }}" aria-required="true">
                </fieldset>
                @error('tagline')
                ...
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Title" name="title" tabindex="0" value="{{ $slide->title }}" aria-required="true">
                </fieldset>
                @error('title')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Subtitle" name="subtitle" tabindex="0" value="{{ $slide->subtitle }}" aria-required="true">
                </fieldset>
                @error('subtitle')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Link" name="link" tabindex="0" value="{{ $slide->link }}" aria-required="true">
                </fieldset>
                @error('link')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
...
@push('scripts')
    <script>
        $(function(){
            // Form Validation
            $('#form-edit-slide').on('submit', function(e) {
                let errors = [];
                ...
                if (errors.length > 0) {
                    ...
                    $('#form-edit-slide .alert-danger').remove();

                    // Add new error messages at the top of the form
                    $('#form-edit-slide').prepend(errorHtml);

                    // Scroll to top
                    $('html, body').animate({
                        scrollTop: $('#form-edit-slide').offset().top - 100
                    }, 500);
