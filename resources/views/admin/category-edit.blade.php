...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" id="form-edit-category" action="{{ route('admin.category.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <fieldset class="name">
                    ...
                    <div class="select w-100">
                        <select name="parent_category" id="parent_category">
                            ...
                            @foreach ($parent_categories as $parent_category)
                                <option value="{{ $parent_category->id }}" @if($parent_category->id === old('parent_category', $category->parent_id)) selected @endif>{{ $parent_category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </fieldset>
        ...
        $(function(){
            ...
            $('#form-edit-category').on('submit', function(e) {
                ...
                if (errors.length > 0) {
                    ...
                    $('#form-edit-category .alert-danger').remove();

                    // Add new error messages at the top of the form
                    $('#form-edit-category').prepend(errorHtml);

                    // Scroll to top
                    $('html, body').animate({
                        scrollTop: $('#form-edit-category').offset().top - 100
                    }, 500);
