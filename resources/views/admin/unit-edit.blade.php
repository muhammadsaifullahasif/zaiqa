...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <!-- new-category -->
        <div class="wg-box">
            <form class="form-new-product form-style-1" id="form-add-unit" action="{{ route('admin.units.update', $unit->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <fieldset class="name">
                    ...
                    <input class="flex-grow" type="text" placeholder="Unit name" name="name"
                        tabindex="0" value="{{ $unit->name }}" aria-required="true">
                </fieldset>
                @error('name')
                ...
                <fieldset class="symbol">
                    ...
                    <input class="flex-grow" type="text" placeholder="Unit Symbol" name="symbol"
                        tabindex="0" value="{{ $unit->symbol }}" aria-required="true">
                </fieldset>
                @error('symbol')
                    <span class="alert alert-danger">{{ $message }}</span>
                @enderror
