...
<main class="pt-90">
                        ...
                        <div class="accordion-body px-0 pb-0 pt-3">
                            <ul class="list list-inline mb-0">
                                @foreach ($categories as $category)
                                    <li class="list-item">
                                        <a href="javascript:void(0)" class="menu-link py-1 w-100 category_filter" data-id="{{ $category->id }}">
                                            <span class="me-auto">{{ $category->name }}</span>
                                            <span class="text-right float-end text-secondary">{{ $category->products->count() }}</span>
                                        </a>
                                    </li>
                                @endforeach
                                {{-- <li class="list-item">
                ...
                <div class="accordion-item mb-4 pb-3">
                    ...
                    <div id="accordion-filter-brand" class="accordion-collapse collapse show border-0" aria-labelledby="accordion-heading-brand" data-bs-parent="#brand-filters">
                        <div class="search-field multi-select accordion-body px-0 pb-0">
                            <select class="brands_filter d-none" multiple name="brands_filter">
                            ...
                            <ul class="multi-select__list list-unstyled">
                                @foreach ($brands as $brand)
                                    <li class="search-suggestion__item multi-select__item text-primary js-search-select js-multi-select brand_filter {{ (in_array($brand->id, explode(',', $filter_brands))) ? 'mult-select__item_selected' : '' }}" data-id="{{ $brand->id }}">
                            ...
                            {{-- <ul class="list list-inline mb-0 brand-list">
                                @foreach ($brands as $brand)
                                    <li class="list-item search-suggestion__item multi-select__item text-primary js-search-select js-multi-select {{ (in_array($brand->id, explode(',', $filter_brands))) ? 'mult-select__item_selected' : '' }}">
                                        <label for="brand_filter_{{ $brand->id }}" class="me-auto menu-link py-1">
                                            <input type="checkbox" id="brand_filter_{{ $brand->id }}" name="brands" value="{{ $brand->id }}" class="chk-brand d-none" @if(in_array($brand->id, explode(',', $filter_brands))) checked="checked" @endif>
                                            {{ $brand->name }}
                                        </label>
        ...
        <div class="shop-list flex-grow-1">
            ...
            <div class="d-flex justify-content-between mb-4 pb-md-2">
                ...
                <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
                    <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0 page_size" aria-label="Page Size" style="margin-right: 20px;">
                        <option value="12" {{ $size == '12' ? 'selected' : '' }}>12</option>
                        <option value="24" {{ $size == '24' ? 'selected' : '' }}>24</option>
                        <option value="24" {{ $size == '50' ? 'selected' : '' }}>50</option>
                        <option value="24" {{ $size == '100' ? 'selected' : '' }}>100</option>
                    </select>

                    <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0 orderBy" aria-label="Sort Items">
                        <option value="-1" {{ $orderBy == '-1' ? 'selected' : '' }}>Default Sorting</option>
                        <option value="low-to-high" {{ $orderBy == 'low-to-high' ? 'selected' : '' }}>Price, low to high</option>
                        <option value="high-to-low" {{ $orderBy == 'high-to-low' ? 'selected' : '' }}>Price, high to low</option>
                        <option value="old-to-new" {{ $orderBy == 'old-to-new' ? 'selected' : '' }}>Date, old to new</option>
                        <option value="new-to-old" {{ $orderBy == 'new-to-old' ? 'selected' : '' }}>Date, new to old</option>
                    </select>
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="{{ route('shop.product.detail', $product->slug) }}"><img loading="lazy" src="{{ asset('uploads/products') }}/{{ $product->image }}" width="330" height="400" alt="{{ $product->name }}" class="pc__img"></a>
                                        </div>
                                        @foreach (explode(',', $product->images) as $gimg)
                                            <div class="swiper-slide">
                                                <a href="{{ route('shop.product.detail', $product->slug) }}"><img loading="lazy" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="330" height="400" alt="{{ $product->name }}" class="pc__img"></a>
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                @if(Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
                                    <a href="{{ route('cart.index') }}" class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium btn btn-warning mb-3">View Cart</a>
                                @else
                            ...
                            <div class="pc__info position-relative">
                                ...
                                <h6 class="pc__title"><a href="{{ route('shop.product.detail', $product->slug) }}">{{ $product->name }}</a></h6>
                                ...
                                @if(Cart::instance('wishlist')->content()->where('id', $product->id)->count() > 0)
                                    <form action="{{ route('wishlist.remove.item', Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId) }}" method="post">
                                        @csrf
                                        @method('delete')
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_2.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_2-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider"
                                    ...
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_3.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_3-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_4.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_4-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_5.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_5-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_6.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_6-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_7.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_7-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_8.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_8-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
                        ...
                        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                            <div class="pc__img-wrapper">
                                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_9.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                        <div class="swiper-slide">
                                            <a href="details.html"><img loading="lazy" src="assets/images/products/product_9-1.jpg" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                                        </div>
                                    </div>
                                    <span class="pc__img-prev"><svg width="7" height="11" viewBox="0 0 7 11" xmlns="http://www.w3.org/2000/svg">
                                            <use href="#icon_prev_sm" />
    ...
    <form action="{{ route('shop.index') }}" method="GET" id="filter_form">
        <input type="hidden" name="orderBy" id="orderBy" value="{{ $orderBy }}">
        <input type="hidden" name="page_size" id="page_size" value="{{ $size }}">
        <input type="hidden" name="brands" id="filter_brand" value="{{ $filter_brands }}">
        <input type="hidden" name="category" id="filter_category" value="{{ $filter_category }}">
        <input type="hidden" name="min_price" id="filter_min" value="{{ $min_price }}">
        <input type="hidden" name="max_price" id="filter_max" value="{{ $max_price }}">
    </form>
</main>
@endsection
    ...
    <script>
        $(function(){
            ...
            $(document).on('click', '.category_filter', function(){
                $('#filter_category').val($(this).data('id'));
                $('#filter_form').submit();
            });
            ...
            $(document).on('change', '.page_size', function(){
                $('#page_size').val($('.page_size option:selected').val());
                $('#filter_form').submit();
            });
            ...
            var brands = `{{ $filter_brands }}`;
            $(document).on('click', '.brand_filter', function(){
                if(brands == '') {
                    brands += $(this).data('id');
                } else {
                    var brands_arr = brands.split(',');
                    if(brands_arr.includes($(this).data('id').toString())) {
                        var index = brands_arr.indexOf($(this).data('id').toString());
                        brands_arr.splice(index,1);
                        brands = brands_arr.join(',');
                    } else {
                        brands += ',' + $(this).data('id');
                    }
                }
