			...
			<div class="swiper-slide">
				<div class="overflow-hidden position-relative h-100">
					<div class="slideshow-character position-absolute bottom-0 pos_right-center">
						<img loading="lazy" src="{{ asset('uploads/slides') }}/{{ $slide->image }}" width="542" height="733" alt="Woman Fashion 1" class="slideshow-character__img animate animate_fade animate_btt animate_delay-9 w-auto h-auto" />
					...
					<div class="swiper-wrapper">
						...
						<div class="swiper-slide">
							<img loading="lazy" class="w-100 h-auto mb-3"
								src="{{ asset('uploads/categories/') }}/{{ $category->image }}" width="124" height="124" alt="{{ $category->name }}" />
							...
							<div class="swiper-wrapper">
								...
								<div class="swiper-slide product-card product-card_style3">
									<div class="pc__img-wrapper">
										<a href="{{ route('shop.product.detail', $sProduct->slug) }}">
											...
											<img loading="lazy" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="258" height="313" alt="{{ $sProduct->name }}" class="pc__img pc__img-second">
											@endforeach
										</a>
									</div>

									<div class="pc__info position-relative">
										<h6 class="pc__title"><a href="{{ route('shop.product.detail', $sProduct->slug) }}">{{ $sProduct->name }}</a>
										</h6>
										<div class="product-card__price d-flex">
											@if ($sProduct->sale_price)
											<span class="money price price-old">${{ $sProduct->regular_price }}</span>
											<span class="money price price-sale">${{ $sProduct->sale_price }}</span>
											@else
												<span class="money price">${{ $sProduct->regular_price }}</span>
											@endif
										</div>
										...
										<div class="anim_appear-bottom position-absolute bottom-0 start-0 d-none d-sm-flex align-items-center bg-body">
											@if(Cart::instance('cart')->content()->where('id', $sProduct->id)->count() > 0)
												<a href="{{ route('cart.index') }}" class="btn-link btn-warning btn-link_lg me-4 text-uppercase fw-medium js-add-cart js-open-aside">View Cart</a>
											@else
											...
											@if(Cart::instance('wishlist')->content()->where('id', $sProduct->id)->count() > 0)
												<form action="{{ route('wishlist.remove.item', Cart::instance('wishlist')->content()->where('id', $sProduct->id)->first()->rowId) }}" method="post">
													@csrf
													@method('delete')
													<button type="submit" class="pc__btn-wl bg-transparent border-0 js-add-wishlist filled-heart" title="Add To Wishlist">
			...
			<div class="row">
				...
				<div class="col-6 col-md-4 col-lg-3">
					<div class="product-card product-card_style3 mb-3 mb-md-4 mb-xxl-5">
						<div class="pc__img-wrapper">
							<a href="{{ route('shop.product.detail', $fProduct->slug) }}">
								<img loading="lazy" src="{{ asset('uploads/products') }}/{{ $fProduct->image }}" width="330" height="400" alt="{{ $fProduct->name }}" class="pc__img">
							</a>
						</div>

						<div class="pc__info position-relative">
							<h6 class="pc__title"><a href="{{ route('shop.product.detail', $fProduct->slug) }}">{{ $fProduct->name }}</a></h6>
							<div class="product-card__price d-flex align-items-center">
								@if ($fProduct->sale_price)
								<span class="money price price-old">${{ $fProduct->regular_price }}</span>
								<span class="money price price-sale">${{ $fProduct->sale_price }}</span>
								@else
									<span class="money price">${{ $fProduct->regular_price }}</span>
								@endif
							</div>
							...
							<div
								...
								@if(Cart::instance('cart')->content()->where('id', $fProduct->id)->count() > 0)
                                    <a href="{{ route('cart.index') }}" class="btn-link btn-link_lg me-4 text-uppercase fw-medium js-add-cart js-open-aside">View Cart</a>
                                @else
                                    <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                                        @csrf
								...
								@if(Cart::instance('wishlist')->content()->where('id', $fProduct->id)->count() > 0)
                                    <form action="{{ route('wishlist.remove.item', Cart::instance('wishlist')->content()->where('id', $fProduct->id)->first()->rowId) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="pc__btn-wl bg-transparent border-0 js-add-wishlist filled-heart" title="Add To Wishlist">
                                            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
